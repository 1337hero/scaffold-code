...
- `Stop` blocks ending with code changed but closeout incomplete (`STATE.md` roll + dated log entry)
...
- `agent_end` sends the agent a follow-up when code changed but closeout is incomplete (STATE
  roll + dated log entry) — once per stale stretch, warning the human on repeats (parity with
  Claude Code's `Stop` gate)
